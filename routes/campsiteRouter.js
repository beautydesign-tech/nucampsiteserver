...
campsiteRouter
  ...
  .put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        console.log("Current campsite:", campsite);
        console.log("Current campsite id:", campsite.comments.id);
        console.log(
          "Current campsite id with params:",
        ...
        if (campsite && campsite.comments.id(req.params.commentId)) {
          ...
          if (
            ...
            err = new Error("Cannot Update! You are not the original author");
            err.status = 403;
            return next(err);
          }
        } else if (!campsite) {
      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          if (
            ...
            err = new Error("Cannot Delete! You are not the original author");
            err.status = 403;
            return next(err);
          }
        } else if (!campsite) {
